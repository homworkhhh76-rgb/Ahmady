...
const defaults={settings:{platformName:'الأحمدي لإدارة الكهرباء',networkName:'',ownerName:'',defaultRate:0,defaultDiscount:0,nearDueDays:3,networkLogo:'',networkLogoKey:'',networkLogoName:'',networkLogoSize:0,supportPhone:''},subscribers:[],regions:[],distributionBoards:[],meters:[],invoices:[],accounts:[],movements:[],logs:[],tasks:[],chats:[],backups:[]};
...
(function(){
  ...
  function renderNetworkSettings(){
    ...
    $('#mainContent').innerHTML=`<div class="page-head"><div>${v23Back()}<h1 style="margin-top:12px">بيانات الشبكة</h1><p>يمكن تعديل كل خانة ثم حفظ البيانات.</p></div></div><form id="networkDataForm" class="settings-detail-card"><div class="form-grid">${inputField('networkName','اسم الشبكة','text',data.settings.networkName||session?.companyName||'','اسم الشبكة')}${inputField('managerAccountName','اسم حساب المدير','text',data.settings.ownerName||session?.ownerName||session?.actorName||'','اسم المدير')}${inputField('networkLoginUsername','اسم المستخدم','text',data.settings.networkLoginUsername||session?.username||'','اسم المستخدم','autocomplete="off"')}${inputField('networkLoginPassword','كلمة المرور','password',data.settings.networkLoginPassword||'','أدخل كلمة المرور المحفوظة')}${inputField('activationCode','رمز التفعيل','text',data.settings.activationCode||session?.subscriptionId||'','رمز التفعيل')}${inputField('supportPhone','رقم مدير التطبيق / الدعم الفني','tel',data.settings.supportPhone||localStorage.getItem('AHMADI_SUPPORT_PHONE_V33')||'','مثال: +97059xxxxxxx','inputmode="tel"')}</div><div class="settings-note">تعديل اسم المستخدم أو كلمة المرور هنا يحدّث بيانات الشبكة المحفوظة داخل النظام والقوالب، ولا يغيّر بيانات التفعيل في القاعدة الأم.</div><div class="modal-actions"><button class="primary-btn" type="submit">${icon('save')} حفظ البيانات</button></div></form>`;
  }
  ...
  handleSubmit=async function(form,submitter=null){
    const fd=new FormData(form),obj=Object.fromEntries(fd.entries());
    try{
      if(form.id==='networkDataForm'){ensureV23Settings();data.settings.networkName=String(obj.networkName||'').trim();data.settings.ownerName=String(obj.managerAccountName||'').trim();data.settings.networkLoginUsername=String(obj.networkLoginUsername||'').trim();data.settings.networkLoginPassword=String(obj.networkLoginPassword||'');data.settings.activationCode=String(obj.activationCode||'').trim();data.settings.supportPhone=String(obj.supportPhone||'').trim();if(data.settings.supportPhone)localStorage.setItem('AHMADI_SUPPORT_PHONE_V33',data.settings.supportPhone);saveSettingsV23();if(session){session.companyName=data.settings.networkName||session.companyName;if(session.actorType==='owner'&&data.settings.ownerName)session.actorName=data.settings.ownerName;localStorage.setItem(SESSION_KEY,JSON.stringify(session))}logAction('تعديل بيانات الشبكة',data.settings.networkName||'بيانات الشبكة');toast('تم حفظ بيانات الشبكة');renderNetworkSettings();return}
  ...
  document.addEventListener('click',async e=>{
   if(e.target.closest?.('[data-v30-action]'))return;
    ...
    const es=e.target.closest('[data-toggle-employee-status]');if(es){e.preventDefault();e.stopPropagation();const next=es.dataset.status==='active'?'inactive':'active';try{await AhmadiCloud.updateEmployee(session.companyId,es.dataset.toggleEmployeeStatus,{status:next});toast(next==='inactive'?'تم تجميد الموظف':'تم إعادة تفعيل الموظف');if(currentPage==='employees'&&!detail)renderEmployees();else render()}catch(err){toast(err.message,'خطأ')}return}
...
(function(){
  ...
  const V24_PHRASES=[
    ...
    ['إدارة شبكات الكهرباء','Electricity Network Management'],
  ...
  function v24EmployeeMeta(row){
    ...
    return {phone:p.phone||'',address:p.address||'',addedDate:p.addedDate||dateOnly(row?.created_at||row?.createdAt||new Date())||dateOnly(new Date()),workDate:p.workDate||'',jobTitle:p.jobTitle||'',loginPassword:p.loginPassword||''};
  }
  ...
  function v24EmployeeMatches(row){const q=detailSearch.trim().toLowerCase(),p=v24EmployeeMeta(row);return !q||`${row.name||''} ${row.username||''} ${p.phone||''} ${p.address||''} ${p.jobTitle||''}`.toLowerCase().includes(q)}
  ...
  handleSubmit=async function(form,submitter=null){
    if(form?.id==='employeeForm'){
      ...
      try{
        ...
        const previousMeta=id?v24EmployeeMeta({id}):{};v24SaveEmployeeMeta(row.id,{phone:String(obj.phone||'').trim(),address:String(obj.address||'').trim(),addedDate:obj.addedDate||dateOnly(new Date()),workDate:obj.workDate||'',jobTitle:String(obj.jobTitle||'').trim(),loginPassword:String(obj.password||'')||previousMeta.loginPassword||''});
...
/* ========================= V32 MAIN UX + SUBSCRIBER ACCOUNTS ========================= */
(function(){
  const V32_NAV_KEY='AHMADI_NAV_ORDER_V32',V32_QUICK_KEY='AHMADI_QUICK_ORDER_V32';
  let v32SubscriberAccountsCache=[];
  const v32SettingsProfiles=()=>{data.settings.subscriberAccountProfiles=data.settings.subscriberAccountProfiles||{};return data.settings.subscriberAccountProfiles};
  const v32Greeting=()=>new Date().getHours()<12?'صباح الخير':'مساء الخير';
  const v32AccountType=()=>session?.actorType==='subscriber'?'حساب مشترك':session?.actorType==='employee'?'حساب موظف':'حساب مدير';
  const v32OwnSubscriberId=()=>String(session?.subscriberId||'');
  const v32IsSubscriber=()=>session?.actorType==='subscriber';
  // Keep subscriber-mode local data isolated from manager/employee caches on shared devices.
  const v32PrevStorageKey=storageKey;
  storageKey=function(ds){return v32IsSubscriber()?`AHMADI_USERS_DATA_V1::${session?.companyId||'guest'}::subscriber::${session?.subscriberId||session?.actorId||'self'}::${ds}`:v32PrevStorageKey(ds)};
  function v32FilterSubscriberDataset(ds,value){
    const id=v32OwnSubscriberId();
    if(ds==='settings'){
      const x=value&&typeof value==='object'&&!Array.isArray(value)?value:{};
      const keys=['platformName','networkName','ownerName','networkLogo','networkLogoKey','networkLogoName','networkLogoSize','appLanguage','supportPhone'];
    ...
    if(ds==='subscribers')return rows.filter(x=>String(x.id)===id);
    if(ds==='meters')return rows.filter(x=>String(x.subscriberId)===id);
    if(ds==='invoices')return rows.filter(x=>String(x.subscriberId)===id);
    if(ds==='movements')return rows.filter(x=>String(x.subscriberId||'')===id||String(x.fromSubscriberId||'')===id||String(x.toSubscriberId||'')===id);
    if(['accounts','logs','tasks','chats','backups'].includes(ds))return [];
    return value;
  }
  const v32PrevAdapter=adapter;
  adapter=function(){const base=v32PrevAdapter();if(!v32IsSubscriber())return base;return {...base,getDataset:ds=>v32FilterSubscriberDataset(ds,data[ds]),setDataset:(ds,v)=>{data[ds]=v32FilterSubscriberDataset(ds,v);normalizeData();save(ds,false)}}};
  const v32ReadJson=(k,fallback)=>{try{const x=JSON.parse(localStorage.getItem(k)||'null');return x??fallback}catch(_){return fallback}};
  const v32SaveJson=(k,v)=>localStorage.setItem(k,JSON.stringify(v));
  const v32FlowForSubscriber=(m,id)=>String(m.subscriberId||'')===String(id)||String(m.fromSubscriberId||'')===String(id)||String(m.toSubscriberId||'')===String(id);
  const v32MovementLabelForSubscriber=(m,id)=>{const t=movementType(m);if(t==='collection')return 'استقبال دفعة';if(t==='send')return 'إرسال دفعة';if(t==='subscriber_transfer')return String(m.fromSubscriberId)===String(id)?'تحويل إلى مشترك':'تحويل من مشترك';if(t==='transfer')return 'تحويل مالي';return moveLabel?.(m)||'حركة مالية'};

  // Remove the old duplicate main icon and add subscriber account management.
  const platformIndex=nav.findIndex(x=>x[0]==='platform');if(platformIndex>=0)nav.splice(platformIndex,1);
  if(!nav.some(x=>x[0]==='subscriberAccounts'))nav.push(['subscriberAccounts','إدارة المشتركين','users','employees.manage']);

  function v32OrderedVisibleNav(){
    let rows=visibleNav().filter(x=>x[0]!=='platform');
    if(v32IsSubscriber())rows=rows.filter(x=>['home','invoices','flows'].includes(x[0]));
    const order=v32ReadJson(V32_NAV_KEY,[]);if(!order.length)return rows;
    const rank=new Map(order.map((id,i)=>[String(id),i]));return [...rows].sort((a,b)=>(rank.get(a[0])??999)-(rank.get(b[0])??999));
  }
  renderNav=function(){
    const drawer=$('#drawerNav');if(!drawer)return;
    const rows=v32OrderedVisibleNav();drawer.innerHTML=rows.map(([id,label,ic])=>`<button class="drawer-link v32-menu-card ${currentPage===id?'active':''}" data-nav="${id}" type="button">${icon(ic)}<span>${label}</span></button>`).join('');
    const bottom=v32IsSubscriber()?[['home','الرئيسية','home'],['invoices','الفواتير','receipt'],['flows','الحركات المالية','wallet']]:[['home','الرئيسية','home'],['subscribers','المشتركين','users'],['meters','العدادات','meter'],['invoices','الفواتير','receipt'],['flows','التدفقات المالية','wallet'],['more','المزيد','more']];
    const core=bottom.filter(x=>x[0]!=='more').map(x=>x[0]);$('#bottomNav').innerHTML=bottom.map(([id,l,ic])=>`<button class="bottom-link ${currentPage===id||(id==='more'&&!core.includes(currentPage))?'active':''}" data-nav="${id}" type="button">${icon(ic)}<span>${l}</span></button>`).join('');hydrateIcons(drawer);hydrateIcons($('#bottomNav'));
  };

  const v32PrevSetSyncStatus=setSyncStatus;
  setSyncStatus=function(status){v32PrevSetSyncStatus(status);const pill=$('#syncPill');if(pill&&!pill.querySelector('.v32-account-type'))pill.insertAdjacentHTML('beforeend',`<small class="v32-account-type">${v32AccountType()}</small>`)};

  function v32SubscriberSummaryBody(){
    const s=stats(),from=data.subscribers.filter(x=>subscriberBalanceScoped(x.id)>0),to=data.subscribers.filter(x=>subscriberBalanceScoped(x.id)<0),net=s.dueFrom-s.dueTo;
    return `<div class="metric-list v32-subscriber-summary">${metric('subscriber-count','عدد المشتركين',num(data.subscribers.length),'مشترك مسجل','line',data.subscribers.map((_,i)=>i+1))}${metric('consumption','إجمالي الاستهلاك',`${num(s.consumption)} KW`,'مجمل الاستهلاك','bars',valuesFor('consumption'))}${metric('due-from','إجمالي الاستحقاق المالي على المشتركين',money(s.dueFrom),`${from.length} مشترك عليهم مبالغ`,'line',from.map(x=>subscriberBalanceScoped(x.id)))}${metric('due-to','إجمالي الاستحقاق المالي للمشتركين',money(s.dueTo),`${to.length} مشترك لهم مبالغ`,'line',to.map(x=>Math.abs(subscriberBalanceScoped(x.id))))}<button class="metric v32-net-metric" data-v32-net-due type="button"><div class="metric-label">صافي المبلغ المستحق من المشتركين</div><div class="metric-value">${money(net)}</div><div class="metric-sub">المستحق عليهم − المستحق لهم</div>${miniChart([s.dueFrom,s.dueTo,net],'bars')}</button></div>`;
  }
  function v32ContactCards(){
    if(v32IsSubscriber())return '';
    const employeeAllowed=session?.actorType==='owner'||hasPerm('employees.view')||hasPerm('employees.manage');
    return `<section class="v32-contact-card"><div class="card-section-title">${icon('message')}<div><strong>التواصل</strong><small>وصول سريع للمنصة والمشتركين والموظفين</small></div></div><div class="v32-contact-grid"><button class="v32-contact-action" data-support-chat="platform" type="button">${icon('message')}<span>التواصل مع المنصة</span></button>${!v32IsSubscriber()?`<button class="v32-contact-action" data-v32-contact="subscribers" type="button">${icon('users')}<span>التواصل مع المشتركين</span></button>`:''}${employeeAllowed&&!v32IsSubscriber()?`<button class="v32-contact-action" data-v32-contact="employees" type="button">${icon('userCog')}<span>التواصل مع الموظفين</span></button>`:''}</div></section>`;
  }
  function v32PostHome(){
    const main=$('#mainContent'),hero=main?.querySelector('.hero');if(!main||!hero)return;
    const actor=session?.actorName||session?.ownerName||'صاحب الحساب',welcome=hero.querySelector('.welcome');if(welcome)welcome.innerHTML=`${v32Greeting()}، <span>${esc(actor)}</span>`;
    if(!hero.querySelector('[data-v32-order]'))hero.insertAdjacentHTML('beforeend',`<button class="v32-order-btn" data-v32-order type="button">${icon('sort')}<span>ترتيب الأيقونات</span></button>`);
    const subs=main.querySelector('[data-summary="subs"]');if(subs){subs.querySelector('.summary-title strong').textContent='ملخص المشتركين';subs.querySelector('.summary-title small').textContent='الاستهلاك والاستحقاقات وصافي المبلغ';subs.querySelector('.summary-body').innerHTML=v32SubscriberSummaryBody();}
    if(!main.querySelector('.v32-contact-card'))main.insertAdjacentHTML('beforeend',v32ContactCards());setSyncStatus(window.AhmadiCloud?.getStatus?.()||{mode:'idle'});hydrateIcons(main);
  }

  function v32ReadonlyQuick(){return `<section class="profile-quick v32-global-quick-card"><div class="profile-quick-title"><strong>إجراءات سريعة</strong><small>اختصارات حساب المشترك</small></div><div class="quick-grid"><button class="quick-action" data-nav="home" type="button"><span class="qa-icon">${icon('home')}</span>بياناتي</button><button class="quick-action" data-nav="invoices" type="button"><span class="qa-icon">${icon('receipt')}</span>فواتيري</button><button class="quick-action" data-nav="flows" type="button"><span class="qa-icon">${icon('wallet')}</span>حركاتي المالية</button></div></section>`}
  function v32RenderSubscriberHome(){
    const id=v32OwnSubscriberId(),sub=subscriberBy(id),snap=subscriberSnapshot(id);setPageTitle('الرئيسية');detail=null;
    $('#mainContent').innerHTML=`<section class="hero"><p class="welcome">${v32Greeting()}، <span>${esc(sub.name||session.actorName||'المشترك')}</span></p><div class="sub">حسابك مخصص للاطلاع على بيانات الاشتراك والفواتير والحركات المالية.</div><div id="syncPill" class="sync-pill"><span class="sync-dot"></span><span>البيانات محدثة</span></div><button class="v32-order-btn" data-v32-order type="button">${icon('sort')}<span>ترتيب الأيقونات</span></button></section>${v32ReadonlyQuick()}<section class="profile-data-card"><div class="card-section-title">${icon('users')}<div><strong>بيانات الاشتراك</strong><small>للاطلاع فقط</small></div></div><div class="profile-data-grid"><span>الاسم<b>${esc(sub.name||'—')}</b></span><span>الهاتف<b dir="ltr">${esc(fullPhone(sub)||'—')}</b></span><span>العنوان<b>${esc(sub.address||'—')}</b></span><span class="debt">المستحق عليك<b>${money(snap.dueFrom)}</b></span><span class="credit">المستحق لك<b>${money(snap.dueTo)}</b></span><span>عدد العدادات<b>${snap.meters.length}</b></span></div></section><section class="electric-summary"><div class="card-section-title">${icon('meter')}<div><strong>ملخص اشتراك الكهرباء</strong><small>من أول قراءة حتى آخر قراءة</small></div></div>${v32ElectricMetrics(snap)}</section>${v32ContactCards()}`;
    setSyncStatus(window.AhmadiCloud?.getStatus?.()||{mode:'idle'});hydrateIcons();
  }
  function v32StaticMetric(label,value,sub=''){return `<div class="metric v32-static-metric"><div class="metric-label">${label}</div><div class="metric-value">${value}</div>${sub?`<div class="metric-sub">${sub}</div>`:''}</div>`}
  function v32ElectricMetrics(snap){return `<div class="metric-list v32-electric-metrics">${v32StaticMetric('تاريخ القراءة الافتتاحية',fmtDate(snap.openingDate))}${v32StaticMetric('القراءة الافتتاحية',`${num(snap.openingReading)} KW`)}${v32StaticMetric('تاريخ آخر قراءة',fmtDate(snap.lastDate))}${v32StaticMetric('الاستهلاك الكلي',`${num(snap.consumption)} KW`)}${v32StaticMetric('إجمالي تكلفة الاستهلاك',money(snap.cost))}${v32StaticMetric('إجمالي المبلغ المدفوع',money(snap.paid))}${v32StaticMetric('إجمالي المبلغ المتبقي عليك',money(snap.dueFrom))}${v32StaticMetric('إجمالي المبلغ المتبقي لك',money(snap.dueTo))}</div>`}

  const v32PrevHome=renderHome;
  renderHome=function(){if(v32IsSubscriber())return v32RenderSubscriberHome();const out=v32PrevHome();v32PostHome();return out};

  const v32PrevSubscribers=renderSubscribers;
  renderSubscribers=function(){
    if(v32IsSubscriber()){renderSubscriberProfile(v32OwnSubscriberId(),'overview');return}
    v32PrevSubscribers();const main=$('#mainContent'),count=main?.querySelector('.count-banner');if(count){const block=document.createElement('div');block.innerHTML=summaryCard('subscriber-page-summary','ملخص المشتركين','اضغط لعرض الإحصاءات التفصيلية','users',v32SubscriberSummaryBody(),true,false);count.replaceWith(block.firstElementChild)}
    const head=main?.querySelector('.page-head');if(head&&!head.querySelector('[data-v32-order]'))head.insertAdjacentHTML('beforeend',`<button class="v32-order-btn" data-v32-order type="button">${icon('sort')}<span>ترتيب الأيقونات</span></button>`);
    if(main&&!main.querySelector('.v32-contact-card')){const summary=main.querySelector('[data-summary="subscriber-page-summary"]');summary?.insertAdjacentHTML('afterend',v32ContactCards())}hydrateIcons(main);
  };

  const v32PrevProfile=renderSubscriberProfile;
  renderSubscriberProfile=function(id,tab=''){
    if(v32IsSubscriber())id=v32OwnSubscriberId();v32PrevProfile(id,tab);const main=$('#mainContent');if(!main)return;const active=detail?.tab||tab||'overview',snap=subscriberSnapshot(id),meters=snap.meters;
    const q=main.querySelector('.profile-quick');if(q){q.classList.add('v32-global-quick-card');const grid=q.querySelector('.profile-actions');grid?.querySelector('[data-profile-action="transfer"]')?.remove();if(grid&&hasPerm('flows.transfer')&&!grid.querySelector('[data-profile-action="subscriber-transfer"]'))grid.insertAdjacentHTML('beforeend',`<button class="quick-action" data-profile-action="subscriber-transfer" data-profile-id="${esc(id)}" type="button"><span class="qa-icon">${icon('users')}</span>تحويل بين حسابات المشتركين</button>`)}
    if(v32IsSubscriber())q?.remove();
    if(active==='overview'){
      const electric=main.querySelector('.electric-summary');if(electric){const ml=electric.querySelector('.metric-list');if(ml)ml.outerHTML=v32ElectricMetrics(snap)}
      const latest=main.querySelector('.latest-invoice-card .card-section-title strong');if(latest)latest.textContent='التفاصيل الكلية لآخر فاتورة';const li=snap.latestInvoice,preview=main.querySelector('.latest-invoice-card .preview-grid');if(li&&preview)preview.innerHTML=`<span>تاريخ الفاتورة<b>${fmtDate(li.date)}</b></span><span>القراءة السابقة<b>${num(li.openingReading)} KW</b></span><span>القراءة الحالية<b>${num(li.closingReading)} KW</b></span><span>الاستهلاك<b>${num(li.consumption)} KW</b></span><span>سعر الكيلو<b>${money(li.unitPrice||0)}</b></span><span>الخصم<b>${money(li.discount||0)}</b></span><span class="highlight">إجمالي الفاتورة<b>${money(invoiceNet(li))}</b></span>`;
    }
    if(active==='invoices'&&!main.querySelector('.v32-profile-meter-summary')){const section=document.createElement('section');section.className='meters-section v32-profile-meter-summary';section.innerHTML=`<div class="section-row-title"><div><strong>عدادات المشترك</strong><small>${meters.length} عداد — التفاصيل الإجمالية</small></div></div><div class="cards-list">${meters.length?meters.map(meterRow).join(''):'<div class="empty">لا توجد عدادات لهذا المشترك.</div>'}</div>`;main.appendChild(section)}
    if(active==='movements'){
      const moves=(data.movements||[]).filter(m=>v32FlowForSubscriber(m,id)).sort((a,b)=>new Date(b.date||b.createdAt||0)-new Date(a.date||a.createdAt||0));const list=main.querySelector('.profile-tab-list');if(list)list.innerHTML=moves.length?moves.map(m=>`<article class="row-card" data-movement="${esc(m.id)}"><span class="row-icon">${icon(movementType(m)==='collection'?'arrowDown':movementType(m)==='send'?'arrowUp':'transfer')}</span><span><strong>${esc(v32MovementLabelForSubscriber(m,id))}</strong><small>${fmtDateTime(m.date||m.createdAt)}${m.notes?` • ${esc(m.notes)}`:''}</small></span><span class="row-amount"><strong>${money(m.amount)}</strong></span></article>`).join(''):'<div class="empty">لا توجد حركات مالية لهذا المشترك.</div>';const tabBtn=main.querySelector('[data-profile-tab="movements"] b');if(tabBtn)tabBtn.textContent=moves.length;
    }
    if(v32IsSubscriber())main.querySelector('#profileBack')?.remove();
    if(!main.querySelector('[data-v32-order]'))main.querySelector('.subscriber-profile-head')?.insertAdjacentHTML('beforeend',`<button class="v32-order-btn" data-v32-order type="button">${icon('sort')}<span>ترتيب الأيقونات</span></button>`);
    hydrateIcons(main);
  };

  const v32PrevInvoices=renderInvoices;
  renderInvoices=function(){
    if(!v32IsSubscriber())return v32PrevInvoices();const id=v32OwnSubscriberId();setPageTitle('الفواتير');let rows=data.invoices.filter(i=>String(i.subscriberId)===id).sort((a,b)=>new Date(b.date||0)-new Date(a.date||0));const q=detailSearch.trim().toLowerCase();if(q)rows=rows.filter(i=>`${i.id||''} ${invoiceNet(i)} ${i.consumption||0} ${meterBy(i.meterId)?.meterNumber||''}`.toLowerCase().includes(q));$('#mainContent').innerHTML=`${pageHead('فواتيري','جميع فواتير اشتراكك للاطلاع فقط',`${rows.length} فاتورة`)}${v32ReadonlyQuick()}${listToolbar('بحث في الفواتير...')}<div class="cards-list">${rows.length?rows.map(invoiceRow).join(''):'<div class="empty">لا توجد فواتير.</div>'}</div>`;v32AddOrderButton();hydrateIcons();
  };
  const v32PrevFlows=renderFlows;
  renderFlows=function(){
    if(!v32IsSubscriber())return v32PrevFlows();const id=v32OwnSubscriberId();setPageTitle('الحركات المالية');let rows=(data.movements||[]).filter(m=>v32FlowForSubscriber(m,id)).sort((a,b)=>new Date(b.date||b.createdAt||0)-new Date(a.date||a.createdAt||0));const q=detailSearch.trim().toLowerCase();if(q)rows=rows.filter(m=>`${v32MovementLabelForSubscriber(m,id)} ${m.notes||''} ${m.amount||0}`.toLowerCase().includes(q));$('#mainContent').innerHTML=`${pageHead('الحركات المالية','عمليات الدفع والاستقبال والتحويل المرتبطة بحسابك',`${rows.length} حركة`)}${v32ReadonlyQuick()}${listToolbar('بحث في الحركات المالية...')}<div class="cards-list">${rows.length?rows.map(m=>`<article class="row-card" data-movement="${esc(m.id)}"><span class="row-icon">${icon(movementType(m)==='collection'?'arrowDown':movementType(m)==='send'?'arrowUp':'transfer')}</span><span><strong>${esc(v32MovementLabelForSubscriber(m,id))}</strong><small>${fmtDateTime(m.date||m.createdAt)}${m.notes?` • ${esc(m.notes)}`:''}</small></span><span class="row-amount"><strong>${money(m.amount)}</strong></span></article>`).join(''):'<div class="empty">لا توجد حركات مالية.</div>'}</div>`;v32AddOrderButton();hydrateIcons();
  };

  function v32QuickDefs(){
    if(v32IsSubscriber())return [];
    if(detail?.kind==='subscriber-profile')return [];
    const defs={subscribers:[['add-subscriber','إضافة مشترك','users'],['add-meter','إضافة عداد','meter'],['follow-subscriber','متابعة مشترك','search']],meters:[['add-meter','إضافة عداد','meter'],['add-board','إضافة لوحة توزيع','panel'],['add-region','إضافة منطقة','mapPin']],invoices:[['add-invoice','إضافة فاتورة','receipt']],flows:[['collection','استقبال دفعة','arrowDown'],['send','إرسال دفعة','arrowUp'],['expense','إضافة مصروف','expense'],['subscriber-transfer','تحويل بين المشتركين','users'],['transfer','تحويل بين الحسابات','transfer']],accounts:[['add-account','إضافة حساب','bank'],['account-transfer','التحويل بين الحسابات المالية','transfer']],employees:[['add-employee','إضافة موظف','userCog']],subscriberAccounts:[['subscriber-account-add','إضافة حساب مشترك','users']],tasks:[['add-task','إضافة مهمة','clipboard']],settings:[['sync','تحديث البيانات','refresh'],['install','تثبيت التطبيق','download']],reports:[['sync','تحديث البيانات','refresh']],logs:[['sync','تحديث البيانات','refresh']]};
    return defs[currentPage]||[['sync','تحديث البيانات','refresh']];
  }
  function v32GlobalQuickCard(){const rows=v32QuickDefs().filter(([a])=>{const need={"add-subscriber":'subscribers.add',"add-meter":'subscribers.add',"add-board":'subscribers.add',"add-region":'subscribers.add',"add-invoice":'subscriptions.add',collection:'flows.collect',send:'flows.send',expense:'flows.expense',transfer:'flows.transfer',"subscriber-transfer":'flows.transfer',"account-transfer":'flows.transfer',"add-account":'accounts.add',"add-employee":'employees.manage',"subscriber-account-add":'employees.manage',"add-task":'tasks.manage'}[a];return !need||hasPerm(need)});if(!rows.length)return '';return `<section class="profile-quick v32-global-quick-card"><div class="profile-quick-title"><strong>إجراءات سريعة</strong><small>أهم إجراءات هذه الصفحة</small></div><div class="quick-grid">${rows.map(([a,l,ic])=>`<button class="quick-action" data-action="${a}" type="button"><span class="qa-icon">${icon(ic)}</span>${l}</button>`).join('')}</div></section>`}
  function v32AddOrderButton(){const main=$('#mainContent');if(!main||main.querySelector('[data-v32-order]'))return;const head=main.querySelector('.page-head');if(head)head.insertAdjacentHTML('beforeend',`<button class="v32-order-btn" data-v32-order type="button">${icon('sort')}<span>ترتيب الأيقونات</span></button>`);else main.insertAdjacentHTML('afterbegin',`<button class="v32-order-btn v32-order-floating" data-v32-order type="button">${icon('sort')}<span>ترتيب الأيقونات</span></button>`)}
  function v32QuickId(el){return el.dataset.action?`a:${el.dataset.action}`:el.dataset.profileAction?`p:${el.dataset.profileAction}`:el.dataset.nav?`n:${el.dataset.nav}`:''}
  function v32ApplyQuickOrder(){const main=$('#mainContent');if(!main)return;const key=detail?.kind||currentPage,orders=v32ReadJson(V32_QUICK_KEY,{}),order=orders[key]||[];main.querySelectorAll('.quick-grid').forEach(grid=>{const children=[...grid.children],rank=new Map(order.map((x,i)=>[x,i]));children.sort((a,b)=>(rank.get(v32QuickId(a))??999)-(rank.get(v32QuickId(b))??999)).forEach(x=>grid.appendChild(x))})}
  function v32PostPage(){
    const main=$('#mainContent');if(!main)return;
    if((currentPage!=='home'||detail)&&!main.querySelector('.profile-quick')){const card=v32GlobalQuickCard();if(card){const head=main.querySelector('.page-head');head?.insertAdjacentHTML('afterend',card)}}
    main.querySelectorAll('.profile-quick,.hero,.v32-contact-card').forEach(x=>x.classList.add('v32-theme-card'));v32AddOrderButton();v32ApplyQuickOrder();const fm=$('#fabMenu');if(fm)fm.classList.add('v32-card-fab-menu');hydrateIcons(main);
  }

  function v32OrderModal(){
    const navRows=v32OrderedVisibleNav(),quick=[...document.querySelectorAll('#mainContent .quick-grid > button')],key=detail?.kind||currentPage;const navOrder=navRows.map(x=>x[0]),qOrder=quick.map(v32QuickId).filter(Boolean),labelByNav=Object.fromEntries(navRows.map(x=>[x[0],x[1]]));
    const rowHtml=(kind,id,label,i,n)=>`<div class="v32-order-row"><span>${esc(label)}</span><div><button type="button" data-v32-move="${kind}" data-id="${esc(id)}" data-dir="-1" ${i===0?'disabled':''}>↑</button><button type="button" data-v32-move="${kind}" data-id="${esc(id)}" data-dir="1" ${i===n-1?'disabled':''}>↓</button></div></div>`;
    openModal('ترتيب الأيقونات والقوائم',`<div class="v32-order-panel"><div class="card-section-title">${icon('sort')}<div><strong>ترتيب القائمة الرئيسية</strong><small>اسحب الترتيب عملياً باستخدام الأسهم؛ الشريط السفلي ثابت حسب الترتيب المطلوب.</small></div></div><div class="v32-order-list">${navOrder.map((id,i)=>rowHtml('nav',id,labelByNav[id]||id,i,navOrder.length)).join('')}</div><div class="card-section-title v32-order-section-title">${icon('more')}<div><strong>ترتيب الإجراءات السريعة</strong><small>خاص بالصفحة الحالية</small></div></div><div class="v32-order-list">${qOrder.length?qOrder.map((id,i)=>{const el=quick.find(x=>v32QuickId(x)===id);return rowHtml('quick',id,el?.textContent?.trim()||id,i,qOrder.length)}).join(''):'<div class="empty">لا توجد إجراءات سريعة قابلة للترتيب في هذه الصفحة.</div>'}</div></div>`);
  }
  function v32MoveOrder(kind,id,dir){
    if(kind==='nav'){const rows=v32OrderedVisibleNav(),arr=rows.map(x=>x[0]),i=arr.indexOf(id),j=i+Number(dir);if(i<0||j<0||j>=arr.length)return;[arr[i],arr[j]]=[arr[j],arr[i]];v32SaveJson(V32_NAV_KEY,arr)}else{const key=detail?.kind||currentPage,orders=v32ReadJson(V32_QUICK_KEY,{}),current=[...document.querySelectorAll('#mainContent .quick-grid > button')].map(v32QuickId).filter(Boolean),arr=orders[key]?.length?[...orders[key]]:current,i=arr.indexOf(id),j=i+Number(dir);if(i<0||j<0||j>=arr.length)return;[arr[i],arr[j]]=[arr[j],arr[i]];orders[key]=arr;v32SaveJson(V32_QUICK_KEY,orders)}renderNav();v32ApplyQuickOrder();closeModal();setTimeout(v32OrderModal,20);
  }

  function v32OpenSubscriberContact(){const rows=[...data.subscribers].sort((a,b)=>String(a.name||'').localeCompare(String(b.name||''),'ar'));openModal('التواصل مع المشتركين',`<div class="cards-list v32-contact-list">${rows.length?rows.map(s=>`<article class="row-card"><span class="row-icon">${icon('users')}</span><span><strong>${esc(s.name)}</strong><small dir="ltr">${esc(fullPhone(s)||'بدون هاتف')}</small></span><div class="v32-inline-actions"><button data-contact="call" data-contact-sub="${esc(s.id)}" type="button">${icon('phone')}</button><button data-contact="whatsapp" data-contact-sub="${esc(s.id)}" type="button">${icon('whatsapp')}</button><button data-contact="sms" data-contact-sub="${esc(s.id)}" type="button">${icon('message')}</button></div></article>`).join(''):'<div class="empty">لا يوجد مشتركون.</div>'}</div>`)}
  async function v32OpenEmployeeContact(){try{const rows=await AhmadiCloud.listEmployees(session.companyId);openModal('التواصل مع الموظفين',`<div class="cards-list v32-contact-list">${rows.length?rows.map(e=>{const p=(data.settings.employeeProfiles||{})[e.id]||{};return `<article class="row-card"><span class="row-icon">${icon('userCog')}</span><span><strong>${esc(e.name)}</strong><small>${esc(p.jobTitle||'')} ${p.phone?`• ${esc(p.phone)}`:''}</small></span><div class="v32-inline-actions"><button data-employee-contact="call" data-employee-phone="${esc(p.phone||'')}" type="button">${icon('phone')}</button><button data-employee-contact="sms" data-employee-phone="${esc(p.phone||'')}" type="button">${icon('message')}</button></div></article>`}).join(''):'<div class="empty">لا يوجد موظفون.</div>'}</div>`)}catch(e){toast(e.message,'خطأ')}}

  function v32AccountProfile(id){return v32SettingsProfiles()[id]||{loginPassword:''}}
  function v32SaveAccountProfile(id,patch){v32SettingsProfiles()[id]={...v32AccountProfile(id),...patch};save('settings')}
  function v32SubscriberAccountCard(row){const sub=subscriberBy(row.subscriber_id||row.subscriberId),p=v32AccountProfile(row.id),active=row.status!=='inactive';return `<article class="employee-card-v24 v32-subscriber-account-card" data-v32-subscriber-account="${esc(row.id)}"><div class="employee-card-head"><span class="employee-avatar-v24">${icon('users')}</span><div class="employee-card-copy"><strong>${esc(sub.name||row.name||'مشترك')}</strong><small>${esc(row.username||'')} • ${active?'نشط':'مجمد'}</small></div><span class="badge ${active?'good':'bad'}">${active?'نشط':'مجمد'}</span></div><div class="employee-card-info"><span><small>اسم الحساب</small><b dir="ltr">${esc(row.username||'—')}</b></span><span><small>كلمة المرور</small><b dir="ltr">${esc(p.loginPassword||'غير محفوظة — حدّثها لإظهارها')}</b></span><span><small>المشترك المرتبط</small><b>${esc(sub.name||row.name||'—')}</b></span><span><small>الحالة</small><b>${active?'فعال':'مجمد'}</b></span></div><div class="employee-card-actions"><button data-v32-subscriber-edit="${esc(row.id)}" type="button">${icon('edit')}<span>تعديل</span></button><button class="${active?'danger-btn':''}" data-v32-subscriber-freeze="${esc(row.id)}" data-status="${esc(row.status||'active')}" type="button">${icon(active?'x':'refresh')}<span>${active?'تجميد':'تفعيل'}</span></button><button class="danger-btn" data-v32-subscriber-delete="${esc(row.id)}" type="button">${icon('trash')}<span>حذف</span></button></div></article>`}
  function renderSubscriberAccounts(){
    if(!(session?.actorType==='owner'||hasPerm('employees.manage'))){currentPage='home';return renderHome()}setPageTitle('إدارة المشتركين');detail=null;$('#mainContent').innerHTML=`${pageHead('إدارة المشتركين','إنشاء حسابات دخول للمشتركين للاطلاع على فواتيرهم وحركاتهم فقط')}<section class="v32-account-note">${icon('users')}<div><strong>حسابات اطلاع فقط</strong><small>المشترك لا يستطيع تعديل البيانات أو إضافة فاتورة أو حركة مالية.</small></div></section>${listToolbar('بحث باسم المشترك أو اسم المستخدم...')}<div id="v32SubscriberAccountsList" class="cards-list"><div class="empty">جاري تحميل الحسابات...</div></div>`;v32PostPage();
    AhmadiCloud.listSubscriberAccounts(session.companyId).then(rows=>{v32SubscriberAccountsCache=rows||[];v32RenderSubscriberAccountRows()}).catch(e=>{const r=$('#v32SubscriberAccountsList');if(r)r.innerHTML=`<div class="empty">${esc(e.message||'تعذر تحميل الحسابات')}</div>`})
  }
  function v32RenderSubscriberAccountRows(){const root=$('#v32SubscriberAccountsList');if(!root)return;const q=detailSearch.trim().toLowerCase(),rows=v32SubscriberAccountsCache.filter(r=>{const s=subscriberBy(r.subscriber_id||r.subscriberId);return !q||`${s.name||r.name||''} ${r.username||''} ${r.status||''}`.toLowerCase().includes(q)});root.innerHTML=rows.length?rows.map(v32SubscriberAccountCard).join(''):'<div class="empty">لا توجد حسابات مشتركين بعد.</div>';hydrateIcons(root)}
  function v32OpenSubscriberAccount(edit=null){
    if(!guard('employees.manage'))return;const subId=edit?.subscriber_id||edit?.subscriberId||'',sub=subscriberBy(subId),items=data.subscribers.map(s=>({value:s.id,label:`${s.name} — ${fullPhone(s)||'بدون هاتف'}`}));
    openModal(edit?'تعديل حساب المشترك':'إضافة حساب مشترك',`<form id="subscriberAccountForm" data-id="${esc(edit?.id||'')}"><div class="form-grid">${pickerField('subscriberId','المشترك',items,subId,subId?sub.name:'اختر المشترك')}${inputField('username','اسم المستخدم','text',edit?.username||'','اسم دخول مستقل','autocomplete="off" autocapitalize="none"')}${inputField('password','كلمة المرور','password','',edit?'اتركها فارغة لعدم التغيير':'4 خانات على الأقل')}<label class="field dropdown-field has-select"><span>الحالة</span><input type="hidden" name="status" value="${edit?.status==='inactive'?'inactive':'active'}"><button class="select-button" data-simple-select="employee-status" type="button"><b>${edit?.status==='inactive'?'مجمد':'نشط'}</b>${icon('chevron')}</button></label></div><div class="modal-actions"><button class="primary-btn" type="submit">${icon('save')} حفظ الحساب</button></div></form>`,'subscriberAccountForm');
  }

  const v32PrevHandleSubmit=handleSubmit;
  handleSubmit=async function(form,submitter=null){
    if(form?.id==='subscriberAccountForm'){
      const fd=new FormData(form),obj=Object.fromEntries(fd.entries()),id=form.dataset.id;try{const sub=subscriberBy(obj.subscriberId);let row;if(id)row=await AhmadiCloud.updateSubscriberAccount(session.companyId,id,{subscriberId:obj.subscriberId,name:sub.name,username:obj.username,password:obj.password,status:obj.status});else row=await AhmadiCloud.createSubscriberAccount(session.companyId,{subscriberId:obj.subscriberId,name:sub.name,username:obj.username,password:obj.password,status:obj.status});if(obj.password)v32SaveAccountProfile(row.id,{loginPassword:String(obj.password)});toast(id?'تم تعديل حساب المشترك':'تم إنشاء حساب المشترك');closeModal();currentPage='subscriberAccounts';renderSubscriberAccounts()}catch(e){toast(e.message||'تعذر حفظ الحساب','خطأ')}return;
    }return v32PrevHandleSubmit(form,submitter)
  };
  const v32PrevHandleAction=handleAction;
  handleAction=function(a){if(a==='subscriber-account-add'){v32OpenSubscriberAccount();return}return v32PrevHandleAction(a)};

  const v32PrevRenderFab=renderFab;
  renderFab=function(){
    if(v32IsSubscriber()){$('#fabRoot').innerHTML='';return}
    if(detail?.kind==='subscriber-profile'){const root=$('#fabRoot'),id=detail.id,actions=[];if(hasPerm('subscriptions.add'))actions.push(['invoice','إضافة فاتورة','receipt']);if(hasPerm('subscribers.add'))actions.push(['meter','إضافة عداد','meter']);if(hasPerm('flows.collect'))actions.push(['collection','استقبال دفعة','arrowDown']);if(hasPerm('flows.send'))actions.push(['send','إرسال دفعة','arrowUp']);if(hasPerm('flows.transfer'))actions.push(['subscriber-transfer','تحويل بين حسابات المشتركين','users']);if(!actions.length){root.innerHTML='';return}root.innerHTML=`<div id="fabMenu" class="fab-menu v32-card-fab-menu hidden">${actions.map(([a,l,ic])=>`<button class="fab-item" data-profile-action="${a}" data-profile-id="${esc(id)}" type="button"><span class="fab-item-icon">${icon(ic)}</span><span>${l}</span></button>`).join('')}</div><button id="fabMain" class="fab-main" type="button">${icon('plus')}</button>`;hydrateIcons(root);return}
    if(currentPage==='subscriberAccounts'&&!detail){const root=$('#fabRoot');root.innerHTML=`<div id="fabMenu" class="fab-menu v32-card-fab-menu hidden"><button class="fab-item" data-action="subscriber-account-add" type="button"><span class="fab-item-icon">${icon('users')}</span><span>إضافة حساب مشترك</span></button></div><button id="fabMain" class="fab-main" type="button">${icon('plus')}</button>`;hydrateIcons(root);return}
    v32PrevRenderFab();const menu=$('#fabMenu');if(currentPage==='subscribers'&&!detail&&menu){if(hasPerm('subscribers.add')&&!menu.querySelector('[data-action="add-meter"]'))menu.insertAdjacentHTML('beforeend',`<button class="fab-item" data-action="add-meter" type="button"><span class="fab-item-icon">${icon('meter')}</span><span>إضافة عداد</span></button>`);if(hasPerm('subscribers.view')&&!menu.querySelector('[data-action="follow-subscriber"]'))menu.insertAdjacentHTML('beforeend',`<button class="fab-item" data-action="follow-subscriber" type="button"><span class="fab-item-icon">${icon('search')}</span><span>متابعة مشترك</span></button>`)}menu?.classList.add('v32-card-fab-menu');if(menu)hydrateIcons(menu)
  };

  const v32PrevStartSession=startSession;
  startSession=async function(freshLogin=false){const out=await v32PrevStartSession(freshLogin);const gs=$('#globalSearchBtn');if(gs)gs.classList.toggle('hidden',v32IsSubscriber());return out};

  const v32PrevRender=render;
  render=function(){
    if(v32IsSubscriber()){
      const allowed=['home','invoices','flows'];if(!allowed.includes(currentPage)&&detail?.kind!=='subscriber-profile'&&detail?.kind!=='invoice-profile')currentPage='home';
      if(detail?.kind==='subscriber-profile')detail.id=v32OwnSubscriberId();
      if(detail?.kind==='invoice-profile'){const inv=data.invoices.find(x=>String(x.id)===String(detail.id));if(!inv||String(inv.subscriberId)!==v32OwnSubscriberId()){detail=null;currentPage='home'}}
    }
    if(currentPage==='subscriberAccounts'&&!detail){renderNav();renderSubscriberAccounts();hydrateIcons();renderFab();return}
    const out=v32PrevRender();queueMicrotask(()=>{v32PostPage();if(currentPage==='home')v32PostHome()});return out
  };

  // Account-specific navigation and UI actions.
  document.addEventListener('click',async e=>{
    const order=e.target.closest?.('[data-v32-order]');if(order){e.preventDefault();e.stopPropagation();v32OrderModal();return}
    const mv=e.target.closest?.('[data-v32-move]');if(mv){e.preventDefault();e.stopPropagation();v32MoveOrder(mv.dataset.v32Move,mv.dataset.id,mv.dataset.dir);return}
    const net=e.target.closest?.('[data-v32-net-due]');if(net){e.preventDefault();e.stopPropagation();const s=stats();openModal('صافي المبلغ المستحق من المشتركين',`<div class="v32-net-breakdown"><div><small>إجمالي المستحق عليهم</small><strong>${money(s.dueFrom)}</strong></div><div><small>إجمالي المستحق لهم</small><strong>${money(s.dueTo)}</strong></div><div class="net"><small>الصافي = عليهم − لهم</small><strong>${money(s.dueFrom-s.dueTo)}</strong></div></div>`);return}
    const contact=e.target.closest?.('[data-v32-contact]');if(contact){e.preventDefault();e.stopPropagation();if(contact.dataset.v32Contact==='subscribers')v32OpenSubscriberContact();else await v32OpenEmployeeContact();return}
    const edit=e.target.closest?.('[data-v32-subscriber-edit]');if(edit){e.preventDefault();e.stopPropagation();const row=v32SubscriberAccountsCache.find(x=>String(x.id)===String(edit.dataset.v32SubscriberEdit));if(row)v32OpenSubscriberAccount(row);return}
    const freeze=e.target.closest?.('[data-v32-subscriber-freeze]');if(freeze){e.preventDefault();e.stopPropagation();try{const status=freeze.dataset.status==='inactive'?'active':'inactive';await AhmadiCloud.updateSubscriberAccount(session.companyId,freeze.dataset.v32SubscriberFreeze,{status});toast(status==='inactive'?'تم تجميد حساب المشترك':'تم تفعيل حساب المشترك');v32SubscriberAccountsCache=await AhmadiCloud.listSubscriberAccounts(session.companyId);v32RenderSubscriberAccountRows()}catch(err){toast(err.message,'خطأ')}return}
    const del=e.target.closest?.('[data-v32-subscriber-delete]');if(del){e.preventDefault();e.stopPropagation();if(confirm('حذف حساب دخول المشترك؟ لن يتم حذف بيانات المشترك أو فواتيره.'))try{await AhmadiCloud.deleteSubscriberAccount(session.companyId,del.dataset.v32SubscriberDelete);delete v32SettingsProfiles()[del.dataset.v32SubscriberDelete];save('settings');toast('تم حذف حساب الدخول');v32SubscriberAccountsCache=await AhmadiCloud.listSubscriberAccounts(session.companyId);v32RenderSubscriberAccountRows()}catch(err){toast(err.message,'خطأ')}return}
  },true);

  // Keep search live on the custom subscriber-accounts page.
  document.addEventListener('input',e=>{if(e.target.id==='listSearch'&&currentPage==='subscriberAccounts'){detailSearch=e.target.value;clearTimeout(window.__v32SearchT);window.__v32SearchT=setTimeout(v32RenderSubscriberAccountRows,100)}},true);
})();


/* ========================= V33 REQUESTED MAIN/LANDING UPDATES ========================= */
(function(){
  const V33_SUPPORT_KEY='AHMADI_SUPPORT_PHONE_V33';
  const V33_LANG_KEY='AHMADI_UI_LANGUAGE_V24';
  const V33_QUICK_KEY='AHMADI_QUICK_ORDER_V32';
  const v33IsSubscriber=()=>session?.actorType==='subscriber';
  const v33Manager=()=>!v33IsSubscriber();
  function v33SafePhone(raw){const s=String(raw||'').trim();return s.replace(/[^+\d]/g,'')}
  function v33RememberSupport(){const ownerSub=session?.actorType==='owner'&&session?.subscriberId?subscriberBy(session.subscriberId):null;const p=v33SafePhone(data?.settings?.supportPhone||fullPhone(ownerSub)||'');if(p)try{localStorage.setItem(V33_SUPPORT_KEY,p)}catch(_){}}
  function v33SupportPhone(){
    const direct=v33SafePhone(localStorage.getItem(V33_SUPPORT_KEY)||data?.settings?.supportPhone||'');if(direct)return direct;
    try{for(let i=0;i<localStorage.length;i++){const k=localStorage.key(i)||'';if(!k.includes('AHMADI_USERS_DATA_V1::')||!k.endsWith('::settings'))continue;const x=JSON.parse(localStorage.getItem(k)||'{}');const p=v33SafePhone(x?.supportPhone||x?.managerPhone||x?.ownerPhone||'');if(p)return p}}catch(_){}
    return '';
  }
  function v33SupportAction(mode){const phone=v33SupportPhone();if(!phone){toast('أضف رقم مدير التطبيق من الإعدادات ← بيانات الشبكة ليعمل الاتصال والواتساب من صفحة الدخول.','تنبيه');return}const digits=phone.replace(/\D/g,'');if(mode==='whatsapp')window.open(`https://wa.me/${digits}`,'_blank');else location.href=`tel:${phone}`}
  function v33LoginUi(){
    const btn=document.getElementById('loginLanguageBtn'),label=document.getElementById('loginLanguageLabel');if(label)label.textContent=(localStorage.getItem(V33_LANG_KEY)||'ar')==='en'?'ع':'EN';
    if(btn&&!btn.dataset.bound){btn.dataset.bound='1';btn.addEventListener('click',()=>{const next=(localStorage.getItem(V33_LANG_KEY)||'ar')==='en'?'ar':'en';localStorage.setItem(V33_LANG_KEY,next);if(data?.settings)data.settings.appLanguage=next;location.reload()})}
  }
  function v33ApplyQuickOrder(root=document){
    let orders={};try{orders=JSON.parse(localStorage.getItem(V33_QUICK_KEY)||'{}')||{}}catch(_){}const key=detail?.kind||currentPage,order=orders[key]||[],rank=new Map(order.map((x,i)=>[x,i]));
    root.querySelectorAll?.('.quick-grid').forEach(grid=>{[...grid.children].sort((a,b)=>{const id=x=>x.dataset.action?`a:${x.dataset.action}`:x.dataset.profileAction?`p:${x.dataset.profileAction}`:x.dataset.nav?`n:${x.dataset.nav}`:'';return (rank.get(id(a))??999)-(rank.get(id(b))??999)}).forEach(x=>grid.appendChild(x))});
  }
  function v33HomeQuickBody(){
    const defs=[
      ['add-subscriber','إضافة مشترك','users','subscribers.add'],['add-meter','إضافة عداد','meter','subscribers.add'],['add-region','إضافة منطقة','mapPin','subscribers.add'],['add-board','إضافة لوحة توزيع','panel','subscribers.add'],['follow-subscriber','متابعة مشترك','search','subscribers.view'],
      ['collection','استقبال دفعة','arrowDown','flows.collect'],['send','إرسال دفعة','arrowUp','flows.send'],['expense','إضافة مصروف','expense','flows.expense'],['subscriber-transfer','تحويل مالي بين حسابات المشتركين','users','flows.transfer'],['transfer','التحويل بين الحسابات المالية','transfer','flows.transfer']
    ].filter(x=>hasPerm(x[3]));
    return `<div class="quick-grid">${defs.map(([a,l,ic])=>`<button class="quick-action" data-action="${a}" type="button"><span class="qa-icon">${icon(ic)}</span><span>${l}</span></button>`).join('')}</div>`
  }
  function v33SubscriberSummary(){
    const s=stats(),from=data.subscribers.filter(x=>subscriberBalanceScoped(x.id)>0),to=data.subscribers.filter(x=>subscriberBalanceScoped(x.id)<0),net=s.dueFrom-s.dueTo;
    return `<div class="metric-list v33-subscriber-summary">${metric('subscriber-count','عدد المشتركين',num(data.subscribers.length),'مشترك مسجل','line',data.subscribers.map((_,i)=>i+1))}${metric('consumption','إجمالي الاستهلاك',`${num(s.consumption)} KW`,'مجمل استهلاك الفواتير','bars',valuesFor('consumption'))}${metric('meter-count','عدد العدادات',num(data.meters.length),`إجمالي استهلاك العدادات ${num(s.consumption)} KW`,'bars',data.meters.map(m=>meterConsumption(m.id,true)))}${metric('consumption-cost','إجمالي ثمن الاستهلاك',money(s.consumptionCost),'قيمة الاستهلاك المسجل','bars',valuesFor('invoice'))}${metric('due-from','إجمالي الاستحقاق المالي على المشتركين',money(s.dueFrom),`${from.length} مشترك عليهم مبالغ`,'line',from.map(x=>subscriberBalanceScoped(x.id)))}${metric('due-to','إجمالي الاستحقاق المالي للمشتركين',money(s.dueTo),`${to.length} مشترك لهم مبالغ`,'line',to.map(x=>Math.abs(subscriberBalanceScoped(x.id))))}${metric('subscriber-net-due','صافي المبلغ المستحق من المشتركين',money(net),'المستحق عليهم − المستحق لهم','bars',[s.dueFrom,s.dueTo,net])}</div>`
  }
  function v33MeterSummary(){
    const s=stats(),cons=data.meters.reduce((a,m)=>a+meterConsumption(m.id,true),0);
    return `<div class="metric-list v33-meter-summary">${metric('meter-count','عدد العدادات',num(data.meters.length),`إجمالي استهلاك العدادات ${num(cons)} KW`,'bars',data.meters.map(m=>meterConsumption(m.id,true)))}${metric('board-count','عدد لوحات التوزيع',num(data.distributionBoards.length),'اضغط لعرض جميع لوحات التوزيع','line',data.distributionBoards.map((_,i)=>i+1))}${metric('region-count','عدد المناطق',num(data.regions.length),'اضغط لعرض جميع المناطق','line',data.regions.map((_,i)=>i+1))}${metric('subscriber-count','عدد المشتركين',num(data.subscribers.length),'اضغط لعرض جميع المشتركين','line',data.subscribers.map((_,i)=>i+1))}</div>`
  }
  function v33FlowsSummary(){
    const s=stats(),all=filtered('movements');
    return `<div class="metric-list v33-flow-summary">${metric('due-from','إجمالي المبلغ المستحق على المشتركين',money(s.dueFrom),'بدون خصم المبالغ المستحقة لهم','line',data.subscribers.map(x=>Math.max(subscriberBalanceScoped(x.id),0)))}${metric('due-to','إجمالي المبالغ المستحقة للمشتركين',money(s.dueTo),'إجمالي الأرصدة لصالح المشتركين','line',data.subscribers.map(x=>Math.max(-subscriberBalanceScoped(x.id),0)))}${metric('all-financial-movements','جميع الحركات المالية',`${all.length} حركة`,'اضغط لعرض كامل الحركات','bars',all.map(x=>Number(x.amount||0)))}</div>`
  }
  function v33MakeSummary(id,title,sub,ic,body){return summaryCard(id,title,sub,ic,body,false,false)}
  function v33EnhanceHome(){
    if(!v33Manager())return;const main=$('#mainContent');if(!main)return;v33RememberSupport();
    const quick=main.querySelector('[data-summary="quick"]');if(quick){quick.classList.remove('open');const b=quick.querySelector('.summary-body');if(b)b.innerHTML=v33HomeQuickBody();const sm=quick.querySelector('.summary-title small');if(sm)sm.textContent='اضغط لعرض أهم العمليات اليومية'}
    const subs=main.querySelector('[data-summary="subs"]');if(subs){subs.querySelector('.summary-title strong').textContent='ملخص المشتركين';subs.querySelector('.summary-title small').textContent='الاستهلاك والعدادات والاستحقاقات';subs.querySelector('.summary-body').innerHTML=v33SubscriberSummary()}
    if(!main.querySelector('[data-summary="meters-summary"]')){const anchor=subs||quick;anchor?.insertAdjacentHTML('afterend',v33MakeSummary('meters-summary','ملخص العدادات','العدادات ولوحات التوزيع والمناطق والمشتركون','meter',v33MeterSummary()))}
    const flows=main.querySelector('[data-summary="flows"]');if(flows){flows.querySelector('.summary-title strong').textContent='قائمة التدفقات المالية';flows.querySelector('.summary-title small').textContent='استحقاقات المشتركين وجميع الحركات المالية';flows.querySelector('.summary-body').innerHTML=v33FlowsSummary()}
    v33ApplyQuickOrder(main);hydrateIcons(main)
  }
  function v33CollapsibleQuick(root=document){
    root.querySelectorAll?.('.profile-quick.v32-global-quick-card').forEach(card=>{card.classList.add('v33-collapsible-quick');if(!card.dataset.v33Init){card.dataset.v33Init='1';card.classList.remove('open');const h=card.querySelector('.profile-quick-title');if(h){h.setAttribute('role','button');h.setAttribute('tabindex','0');h.insertAdjacentHTML('beforeend',`<span class="v33-quick-chevron">${icon('chevron')}</span>`)}}});
  }
  function v33RenameTransfers(root=document){root.querySelectorAll?.('button').forEach(b=>{const t=b.textContent.trim();if(t==='تحويل بين الحسابات'||t==='تحويل بين الحسابات المالية') {const span=b.querySelector('span:last-child');if(span)span.textContent='التحويل بين الحسابات المالية';else [...b.childNodes].filter(n=>n.nodeType===3).forEach(n=>n.textContent=' التحويل بين الحسابات المالية')}})}
  function v33Post(){v33LoginUi();const main=$('#mainContent');if(!main)return;if(currentPage==='home'&&!detail)v33EnhanceHome();v33CollapsibleQuick(main);v33RenameTransfers(main);v33ApplyQuickOrder(main);hydrateIcons(main)}

  function v33FilteredMeters(){const q=detailSearch.trim().toLowerCase();let rows=data.meters.filter(m=>meterInRange(m)).filter(m=>!q||`${m.label||''} ${m.meterNumber||''} ${subscriberBy(m.subscriberId).name||''} ${regionBy(m.regionId).name||''} ${boardBy(m.boardId).name||''}`.toLowerCase().includes(q));if(detailSort==='amount-desc')rows.sort((a,b)=>meterConsumption(b.id,true)-meterConsumption(a.id,true));else if(detailSort==='amount-asc')rows.sort((a,b)=>meterConsumption(a.id,true)-meterConsumption(b.id,true));else if(detailSort==='date-asc')rows.sort((a,b)=>new Date(a.createdAt||a.openingDate||0)-new Date(b.createdAt||b.openingDate||0));else rows.sort((a,b)=>new Date(b.createdAt||b.openingDate||0)-new Date(a.createdAt||a.openingDate||0));return rows}
  function v33CustomDetail(kind){
    let title='',total='',rows=[],rowsHtml='';const q=detailSearch.trim().toLowerCase();
    if(kind==='meter-count'){title='جميع العدادات';rows=v33FilteredMeters();const cons=rows.reduce((a,m)=>a+meterConsumption(m.id,true),0);total=`${rows.length} عداد • ${num(cons)} KW`;rowsHtml=rows.map(meterRow).join('')}
    else if(kind==='board-count'){title='لوحات التوزيع';rows=data.distributionBoards.filter(b=>!q||`${b.name||''} ${b.boardNumber||''} ${regionBy(b.regionId).name||''}`.toLowerCase().includes(q));if(detailSort==='date-asc')rows.sort((a,b)=>new Date(a.createdAt||0)-new Date(b.createdAt||0));else rows.sort((a,b)=>new Date(b.createdAt||0)-new Date(a.createdAt||0));total=`${rows.length} لوحة توزيع`;rowsHtml=rows.map(distributionBoardRow).join('')}
    else if(kind==='region-count'){title='المناطق';rows=data.regions.filter(r=>!q||`${r.name||''} ${r.regionNumber||''}`.toLowerCase().includes(q));if(detailSort==='date-asc')rows.sort((a,b)=>new Date(a.createdAt||0)-new Date(b.createdAt||0));else rows.sort((a,b)=>new Date(b.createdAt||0)-new Date(a.createdAt||0));total=`${rows.length} منطقة`;rowsHtml=rows.map(regionRow).join('')}
    else if(kind==='all-financial-movements'){title='جميع الحركات المالية';rows=sortRows(filtered('movements').filter(m=>!q||`${subscriberBy(m.subscriberId).name||''} ${subscriberBy(m.fromSubscriberId).name||''} ${subscriberBy(m.toSubscriberId).name||''} ${m.notes||''} ${m.payee||''} ${m.amount||''}`.toLowerCase().includes(q)),'amount','date');total=`${rows.length} حركة • ${money(rows.reduce((a,m)=>a+Number(m.amount||0),0))}`;rowsHtml=rows.map(flowRow).join('')}
    else if(kind==='subscriber-net-due'){const s=stats();title='صافي المبلغ المستحق من المشتركين';total=money(s.dueFrom-s.dueTo);rowsHtml=`<div class="v32-net-breakdown"><div><small>إجمالي المستحق عليهم</small><strong>${money(s.dueFrom)}</strong></div><div><small>إجمالي المستحق لهم</small><strong>${money(s.dueTo)}</strong></div><div class="net"><small>الصافي = عليهم − لهم</small><strong>${money(s.dueFrom-s.dueTo)}</strong></div></div>`}
    else return false;
    setPageTitle(title);$('#mainContent').innerHTML=`<div class="page-head"><div><button class="secondary-btn" id="detailBack" type="button">${icon('back')} رجوع</button><h1 style="margin-top:12px">${esc(title)}</h1></div></div>${kind==='subscriber-net-due'?'':timeFilterHtml()+listToolbar('بحث في النتائج...')}<div class="count-banner"><div><small>الإجمالي</small><strong>${total}</strong></div></div><div class="cards-list">${rowsHtml||'<div class="empty">لا توجد بيانات.</div>'}</div>`;return true
  }
  const prevDetail=renderDetail;renderDetail=function(){if(v33CustomDetail(detail?.kind))return;return prevDetail()};
  const prevRender=render;render=function(){const out=prevRender();queueMicrotask(v33Post);return out};
  const prevStartSession=startSession;startSession=async function(fresh=false){const out=await prevStartSession(fresh);v33RememberSupport();queueMicrotask(v33Post);return out};

  document.addEventListener('click',e=>{
    const sup=e.target.closest?.('[data-v33-login-support]');if(sup){e.preventDefault();e.stopPropagation();v33SupportAction(sup.dataset.v33LoginSupport);return}
    const h=e.target.closest?.('.v33-collapsible-quick .profile-quick-title');if(h){e.preventDefault();h.closest('.v33-collapsible-quick')?.classList.toggle('open');return}
  },true);
  document.addEventListener('keydown',e=>{const h=e.target.closest?.('.v33-collapsible-quick .profile-quick-title');if(h&&(e.key==='Enter'||e.key===' ')){e.preventDefault();h.closest('.v33-collapsible-quick')?.classList.toggle('open')}},true);
  v33LoginUi();
})();
