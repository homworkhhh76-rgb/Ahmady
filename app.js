...
(function(){
  ...
  handleSubmit=async function(form,submitter=null){
    const fd=new FormData(form),obj=Object.fromEntries(fd.entries());
    try{
      if(form.id==='networkDataForm'){ensureV23Settings();data.settings.networkName=String(obj.networkName||'').trim();data.settings.ownerName=String(obj.managerAccountName||'').trim();data.settings.networkLoginUsername=String(obj.networkLoginUsername||'').trim();data.settings.networkLoginPassword=String(obj.networkLoginPassword||'');data.settings.activationCode=String(obj.activationCode||'').trim();data.settings.supportPhone=String(obj.supportPhone||'').trim();if(data.settings.supportPhone)localStorage.setItem('AHMADI_SUPPORT_PHONE_V34',data.settings.supportPhone);saveSettingsV23();if(session){session.companyName=data.settings.networkName||session.companyName;if(session.actorType==='owner'&&data.settings.ownerName)session.actorName=data.settings.ownerName;localStorage.setItem(SESSION_KEY,JSON.stringify(session))}logAction('تعديل بيانات الشبكة',data.settings.networkName||'بيانات الشبكة');toast('تم حفظ بيانات الشبكة');renderNetworkSettings();return}
...
/* ========================= V34 ADD ONLY — restore V32 + additive V33 features ========================= */
(function(){
  const V34_SUPPORT_KEY='AHMADI_SUPPORT_PHONE_V34';
  const V34_LANG_KEY='AHMADI_UI_LANGUAGE_V24';
  const v34IsSubscriber=()=>session?.actorType==='subscriber';
  const v34SafePhone=raw=>String(raw||'').trim().replace(/[^+\d]/g,'');
  function v34RememberSupport(){const p=v34SafePhone(data?.settings?.supportPhone||'');if(p)try{localStorage.setItem(V34_SUPPORT_KEY,p)}catch(_){}}
  function v34SupportPhone(){
    const direct=v34SafePhone(localStorage.getItem(V34_SUPPORT_KEY)||data?.settings?.supportPhone||'');if(direct)return direct;
    try{for(let i=0;i<localStorage.length;i++){const k=localStorage.key(i)||'';if(!k.includes('AHMADI_USERS_DATA_V1::')||!k.endsWith('::settings'))continue;const x=JSON.parse(localStorage.getItem(k)||'{}');const p=v34SafePhone(x?.supportPhone||x?.managerPhone||x?.ownerPhone||'');if(p)return p}}catch(_){}
    return '';
  }
  function v34SupportAction(mode){const phone=v34SupportPhone();if(!phone){toast('أضف رقم مدير التطبيق من الإعدادات ← بيانات الشبكة ليعمل الاتصال والواتساب من صفحة الدخول.','تنبيه');return}const digits=phone.replace(/\D/g,'');if(mode==='whatsapp')window.open(`https://wa.me/${digits}`,'_blank');else location.href=`tel:${phone}`}
  function v34LoginUi(){
    const btn=document.getElementById('loginLanguageBtn'),label=document.getElementById('loginLanguageLabel');if(label)label.textContent=(localStorage.getItem(V34_LANG_KEY)||'ar')==='en'?'ع':'EN';
    if(btn&&!btn.dataset.bound){btn.dataset.bound='1';btn.addEventListener('click',()=>{const next=(localStorage.getItem(V34_LANG_KEY)||'ar')==='en'?'ar':'en';localStorage.setItem(V34_LANG_KEY,next);if(data?.settings)data.settings.appLanguage=next;location.reload()})}
  }
  function v34MeterSummary(){
    const cons=data.meters.reduce((a,m)=>a+meterConsumption(m.id,true),0);
    return `<div class="metric-list v34-meter-summary">${metric('meter-count','عدد العدادات',num(data.meters.length),`إجمالي استهلاك العدادات ${num(cons)} KW`,'bars',data.meters.map(m=>meterConsumption(m.id,true)))}${metric('board-count','عدد لوحات التوزيع',num(data.distributionBoards.length),'اضغط لعرض جميع لوحات التوزيع','line',data.distributionBoards.map((_,i)=>i+1))}${metric('region-count','عدد المناطق',num(data.regions.length),'اضغط لعرض جميع المناطق','line',data.regions.map((_,i)=>i+1))}${metric('subscriber-count','عدد المشتركين',num(data.subscribers.length),'اضغط لعرض جميع المشتركين','line',data.subscribers.map((_,i)=>i+1))}</div>`;
  }
  function v34ExtraFlowsSummary(){
    const s=stats(),all=filtered('movements');
    return `<div class="metric-list v34-extra-flow-summary">${metric('due-from','إجمالي المبلغ المستحق على المشتركين',money(s.dueFrom),'بدون خصم المبالغ المستحقة لهم','line',data.subscribers.map(x=>Math.max(subscriberBalanceScoped(x.id),0)))}${metric('due-to','إجمالي المبالغ المستحقة للمشتركين',money(s.dueTo),'إجمالي الأرصدة لصالح المشتركين','line',data.subscribers.map(x=>Math.max(-subscriberBalanceScoped(x.id),0)))}${metric('all-financial-movements','جميع الحركات المالية',`${all.length} حركة`,'اضغط لعرض كامل الحركات','bars',all.map(x=>Number(x.amount||0)))}</div>`;
  }
  function v34AddHomeCards(){
    if(v34IsSubscriber()||currentPage!=='home'||detail)return;const main=$('#mainContent');if(!main)return;v34RememberSupport();
    const grid=main.querySelector('.summary-grid');if(!grid)return;
    const quick=grid.querySelector('[data-summary="quick"]');if(quick){quick.classList.remove('open');const sm=quick.querySelector('.summary-title small');if(sm)sm.textContent='اضغط لعرض أهم العمليات اليومية'}
    if(!grid.querySelector('[data-summary="meters-summary"]'))grid.insertAdjacentHTML('beforeend',summaryCard('meters-summary','ملخص العدادات','العدادات ولوحات التوزيع والمناطق والمشتركون','meter',v34MeterSummary(),false,false));
    if(!grid.querySelector('[data-summary="extra-financial-flows"]'))grid.insertAdjacentHTML('beforeend',summaryCard('extra-financial-flows','قائمة التدفقات المالية','استحقاقات المشتركين وجميع الحركات المالية','wallet',v34ExtraFlowsSummary(),false,false));
    hydrateIcons(grid);
  }
  function v34CollapsibleQuick(root=document){
    root.querySelectorAll?.('.profile-quick.v32-global-quick-card').forEach(card=>{card.classList.add('v34-collapsible-quick');if(!card.dataset.v34Init){card.dataset.v34Init='1';card.classList.remove('open');const h=card.querySelector('.profile-quick-title');if(h){h.setAttribute('role','button');h.setAttribute('tabindex','0');h.insertAdjacentHTML('beforeend',`<span class="v34-quick-chevron">${icon('chevron')}</span>`)}}});
  }
  function v34FilteredMeters(){const q=detailSearch.trim().toLowerCase();let rows=data.meters.filter(m=>meterInRange(m)).filter(m=>!q||`${m.label||''} ${m.meterNumber||''} ${subscriberBy(m.subscriberId).name||''} ${regionBy(m.regionId).name||''} ${boardBy(m.boardId).name||''}`.toLowerCase().includes(q));if(detailSort==='amount-desc')rows.sort((a,b)=>meterConsumption(b.id,true)-meterConsumption(a.id,true));else if(detailSort==='amount-asc')rows.sort((a,b)=>meterConsumption(a.id,true)-meterConsumption(b.id,true));else if(detailSort==='date-asc')rows.sort((a,b)=>new Date(a.createdAt||a.openingDate||0)-new Date(b.createdAt||b.openingDate||0));else rows.sort((a,b)=>new Date(b.createdAt||b.openingDate||0)-new Date(a.createdAt||a.openingDate||0));return rows}
  function v34CustomDetail(kind){
    let title='',total='',rows=[],rowsHtml='';const q=detailSearch.trim().toLowerCase();
    if(kind==='meter-count'){title='جميع العدادات';rows=v34FilteredMeters();const cons=rows.reduce((a,m)=>a+meterConsumption(m.id,true),0);total=`${rows.length} عداد • ${num(cons)} KW`;rowsHtml=rows.map(meterRow).join('')}
    else if(kind==='board-count'){title='لوحات التوزيع';rows=data.distributionBoards.filter(b=>!q||`${b.name||''} ${b.boardNumber||''} ${regionBy(b.regionId).name||''}`.toLowerCase().includes(q));rows=detailSort==='date-asc'?[...rows].sort((a,b)=>new Date(a.createdAt||0)-new Date(b.createdAt||0)):[...rows].sort((a,b)=>new Date(b.createdAt||0)-new Date(a.createdAt||0));total=`${rows.length} لوحة توزيع`;rowsHtml=rows.map(distributionBoardRow).join('')}
    else if(kind==='region-count'){title='المناطق';rows=data.regions.filter(r=>!q||`${r.name||''} ${r.regionNumber||''}`.toLowerCase().includes(q));rows=detailSort==='date-asc'?[...rows].sort((a,b)=>new Date(a.createdAt||0)-new Date(b.createdAt||0)):[...rows].sort((a,b)=>new Date(b.createdAt||0)-new Date(a.createdAt||0));total=`${rows.length} منطقة`;rowsHtml=rows.map(regionRow).join('')}
    ...
    setPageTitle(title);$('#mainContent').innerHTML=`<div class="page-head"><div><button class="secondary-btn" id="detailBack" type="button">${icon('back')} رجوع</button><h1 style="margin-top:12px">${esc(title)}</h1></div></div>${timeFilterHtml()+listToolbar('بحث في النتائج...')}<div class="count-banner"><div><small>الإجمالي</small><strong>${total}</strong></div></div><div class="cards-list">${rowsHtml||'<div class="empty">لا توجد بيانات.</div>'}</div>`;return true;
  }
  const v34PrevDetail=renderDetail;renderDetail=function(){if(v34CustomDetail(detail?.kind))return;return v34PrevDetail()};
  const v34PrevRender=render;render=function(){const out=v34PrevRender();queueMicrotask(()=>{v34LoginUi();v34AddHomeCards();v34CollapsibleQuick($('#mainContent')||document);hydrateIcons()});return out};
  const v34PrevStartSession=startSession;startSession=async function(fresh=false){const out=await v34PrevStartSession(fresh);v34RememberSupport();queueMicrotask(()=>{v34LoginUi();v34AddHomeCards();v34CollapsibleQuick($('#mainContent')||document)});return out};
  document.addEventListener('click',e=>{
    const sup=e.target.closest?.('[data-v34-login-support]');if(sup){e.preventDefault();e.stopPropagation();v34SupportAction(sup.dataset.v34LoginSupport);return}
    const h=e.target.closest?.('.v34-collapsible-quick .profile-quick-title');if(h){e.preventDefault();h.closest('.v34-collapsible-quick')?.classList.toggle('open');return}
  },true);
  document.addEventListener('keydown',e=>{const h=e.target.closest?.('.v34-collapsible-quick .profile-quick-title');if(h&&(e.key==='Enter'||e.key===' ')){e.preventDefault();h.closest('.v34-collapsible-quick')?.classList.toggle('open')}},true);
  v34LoginUi();
})();
